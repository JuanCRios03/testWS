...
public class Service {
    ...
    @Produces(MediaType.APPLICATION_XML)
    public String getSalarioPromedioInXML() {
        ...
        if (totalPersonas > 0) {
            salarioPromedio = salarioTotal / totalPersonas;
        }else{
            salarioPromedio = 0;
        }
        return "<averageSalary>" + salarioPromedio + "</averageSalary>";
    }
...
//    }
    @GET
    @Path("/getSalarioTotalInJSON")
    @Produces(MediaType.APPLICATION_JSON)
    public double getSalarioTotalnJSON() {
        double salarioTotal = 0;
        ...
        return salarioTotal; // Retorna la suma
    }
}